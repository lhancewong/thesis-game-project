...
func _on_point_entered(point: Point, function: Function, props: Dictionary = {}) -> void:
	...
	var x_value: String = (
		point.value.x
		if point.value.x is String
		else ECUtilities._format_value(point.value.x, ECUtilities._is_decimal(point.value.x))
	)
	var y_value: String = (
		point.value.y
		if point.value.y is String
		else ECUtilities._format_value(point.value.y, ECUtilities._is_decimal(point.value.y))
	)
	var color: Color = (
		function.get_color()
		if function.get_type() != Function.Type.PIE
