...
func _draw() -> void:
	...
	match self.type:
		...
		Function.Type.PIE:
			draw_rect(
				Rect2(center - (Vector2.ONE * 3), Vector2.ONE * 3 * 2), color, true, 1.0, false
			)
	...
	match marker:
		...
		Function.Marker.CROSS:
			draw_line(center - (Vector2.ONE * 3), center + (Vector2.ONE * 3), color, 3, true)
			draw_line(
				center + (Vector2(1, -1) * 3), center + (Vector2(-1, 1) * 3), color, 3 / 2, true
			)
