...
static func random_float_range(from: float, to: float, size: Vector2, _seed: int = 1234) -> Matrix:
	...
	for row in range(size.x):
		...
		for column in range(size.y):
			matrix_row.append(rand_range(from, to))
...
static func sub_matrix(_matrix: Matrix, from: PoolIntArray, to: PoolIntArray) -> Matrix:
	assert(
		not (to[0] > _matrix.rows() or to[1] > _matrix.columns()),
		(
			"%s is not an acceptable size for the submatrix, giving a matrix of size %s"
			% [to, _matrix.get_size()]
		)
	)
	var array: Array = []
...
static func dot(_matrix1: Matrix, _matrix2: Matrix) -> Matrix:
	if _matrix1.get_size().y != _matrix2.get_size().x:
		printerr(
			(
				"matrix1 number of columns: %s must be the same as matrix2 number of rows: %s"
				% [_matrix1.get_size().y, _matrix2.get_size().x]
			)
		)
		return Matrix.new()
	var array: Array = []
...
static func hadamard(_matrix1: Matrix, _matrix2: Matrix) -> Matrix:
	if _matrix1.get_size() != _matrix2.get_size():
		printerr(
			(
				"matrix1 size: %s must be the same as matrix2 size: %s"
				% [_matrix1.get_size(), _matrix2.get_size()]
			)
		)
		return Matrix.new()
	var array: Array = []
	for x in range(_matrix1.to_array().size()):
		...
		for y in range(_matrix1.to_array()[x].size()):
			assert(
				(
					typeof(_matrix1.to_array()[x][y]) != TYPE_STRING
					and typeof(_matrix2.to_array()[x][y]) != TYPE_STRING
				),
				"can't apply operations over a Matrix of Strings"
			)
