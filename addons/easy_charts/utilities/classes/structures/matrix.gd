...
func get_column(column: int) -> Array:
	assert(
		column < columns(),
		"index of the column requested (%s) exceedes matrix columns (%s)" % [column, columns()]
	)
...
func get_row(row: int) -> Array:
	assert(
		row < rows(), "index of the row requested (%s) exceedes matrix rows (%s)" % [row, rows()]
	)
	return values[row]
	...
	for row_i in values.size():
		for column_i in values[row_i].size():
			string += (
				"%*s"
				% [
					last_string_len + 1 if column_i != 0 else last_string_len,
					values[row_i][column_i]
				]
			)
		string += "\n"
	return string
