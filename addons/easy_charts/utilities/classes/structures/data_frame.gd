...
func _init(
	...
	if datamatrix.empty():
		datamatrix.resize(labels.size(), headers.size())
	if labels.empty():
		for label in range(datamatrix.get_size().x):
			labels.append(label as String)
	if headers.empty():
		for header in range(datamatrix.get_size().y):
			headers.append(MatrixGenerator.get_letter_index(header))
	build_dataframe(datamatrix, headers, labels, table_name)


func build_dataframe(
	datamatrix: Matrix,
	headers: PoolStringArray = [],
	labels: PoolStringArray = [],
	table_name: String = ""
...
func build_dataset_from_matrix(
	datamatrix: Matrix, headers: PoolStringArray, labels: PoolStringArray
) -> Array:
...
func build_dataset(data: Array, headers: PoolStringArray, labels: PoolStringArray) -> Array:
	...
	for row_i in range(labels.size()):
		dataset.append(([labels[row_i]] + data[row_i]) if not data.empty() else [labels[row_i]])
	return dataset


func insert_column(column: Array, header: String = "", index: int = dataset[0].size() - 1) -> void:
	assert(
		column.size() == (datamatrix.rows() if not datamatrix.empty() else labels.size()),
		"error: the column size must match the dataset column size"
	)
...
func insert_row(row: Array, label: String = "", index: int = dataset.size() - 1) -> PoolStringArray:
	assert(
		row.size() == (datamatrix.columns() if not datamatrix.empty() else headers.size()),
		"error: the row size must match the dataset row size"
	)
...
func _to_string() -> String:
	...
	string += "\n['{table_name}' : {rows} rows x {columns} columns]\n".format(
		{rows = datamatrix.rows(), columns = datamatrix.columns(), table_name = table_name}
	)
	return string
	...
	if ":" in _property:
		...
		if (property[0]).is_valid_integer():
			...
			if property[1] == "*":
				...
				return get_icolumns(
					range(get_column_index(property[0]), get_column_index(property[1]))
				)
	# ";" --> Rows
	elif ";" in _property:
		var property: PoolStringArray = _property.split(";")
