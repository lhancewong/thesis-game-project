...
func _ready():
	...
	var y2: Array = ArrayOperations.add_float(
		ArrayOperations.multiply_int(ArrayOperations.sin(x), 20), 20
	)
	var y3: Array = ArrayOperations.add_float(
		ArrayOperations.multiply_int(ArrayOperations.cos(x), -5), -3
	)
	...
	f1 = Function.new(
		x,
		y,
		"Pressure",  # This will create a function with x and y values taken by the Arrays
