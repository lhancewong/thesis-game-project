		...
		match function.get_type():
			Function.Type.PIE:
				for i in function.x.size():
					var interp_color: Color = function.get_gradient().interpolate(
						float(i) / float(function.x.size())
					)
					function_legend.add_label(
						function.get_type(), interp_color, Function.Marker.NONE, function.y[i]
					)
			_:
...
func calculate_domain(values: Array) -> Dictionary:
	...
	return {
		...
		ub = ECUtilities._round_max(min_max.max),
		has_decimals = ECUtilities._has_decimals(values)
	}


func update_gridbox(
	x_domain: Dictionary, y_domain: Dictionary, x_labels: PoolStringArray, y_labels: PoolStringArray
) -> void:
...
func calculate_plotbox_margins(x_domain: Dictionary, y_domain: Dictionary) -> Vector2:
	var plotbox_margins: Vector2 = Vector2(
		chart_properties.x_tick_size, chart_properties.y_tick_size
	)
	...
	if chart_properties.show_tick_labels:
		...
		if y_domain.lb < 0:  # negative number
			var y_min_formatted: String = ECUtilities._format_value(
				y_domain.lb, y_domain.has_decimals
			)
