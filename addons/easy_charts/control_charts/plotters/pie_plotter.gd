...
func sample(radius: float, center: Vector2, total: float, ratios: PoolRealArray) -> void:
	...
	for ratio in ratios:
		...
		slices.append(_calc_circle_arc_poly(center, radius, start_angle, end_angle))
...
func _draw_pie() -> void:
	for i in slices.size():
		draw_colored_polygon(
			slices[i], function.get_gradient().interpolate(float(i) / float(slices.size() - 1))
		)
		...
		for i in slices.size():
			if Geometry.is_point_in_polygon(get_relative_position(event.position), slices[i]):
				var point: Point = Point.new(
					self.box.get_center() + slices_dirs[i] * self.radius * 0.5,
				...
				if focused_point == point:
					...
					emit_signal(
						...
						focused_point,
						function,
						{interpolation_index = float(i) / float(slices.size() - 1)}
					)
					return
